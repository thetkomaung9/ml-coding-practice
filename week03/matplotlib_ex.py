...
plt.title('Correlatiomn Heatmap of Titanic')
...
# 나이대별 생존자와 시망자 수 계산하기 위해 벙주형 변수로 변화
...
# 범례 설정
plt.legend(violin_plot['bodies'], ['1st Class', '2nd Classs', '3rd Class'],
...
"""* 에러바는 데이터의 표준 편차를 나타내는 경우에는 길이가 길수록 해당 그룹의 데이터가 퍼져 있음을 의미합니다."""
...
# pt.subplot() 함수를 사용하여 하나의 그림의 figure에 개별 서브플롯을 하나씩 추가
# 첫 번째 서브플롯 성정(선 그래프)
plt.subplot(2, 1, 1)
# 선 그래프 그리기
...
# 제목 설정
fig.suptitle('Survival Analysis by Number of Parents/Children (Parch) on Titanic')
plt.tight_layout()
...
ax2.set_ylabel('Survived Count', color='deeppink') # y축의 레이블
ax2.tick_params(axis='y', labelcolor='deeppink') # y축 눈금 색상 설정
ax2.legend(loc='upper right', bbox_to_anchor=(1, 0.9)) # 범례